...
$app->get('/comentarios', function() use ($app){
    ...
    $pdo=$app->db;
    $r = $pdo->query("select id, nombre, apellido from contacto")->fetchAll(PDO::FETCH_ASSOC);
...
$app->post('/guardarSugerencia', function() use ($app){
	...
	if($valores['id']){
		$sql = "update contacto set NOMBRE=:nombre, APELLIDO=:apellido, EMAIL=:email, CURSO=:curso WHERE ID=:id";
		$pdo=$app->db;
		$q = $pdo->prepare($sql);
		$q->execute($valores);
	...
	else
	{
		...
		$sql = "INSERT INTO contacto (nombre, apellido, email, curso) VALUES (:nombre, :apellido, :email, :curso)";
		$pdo=$app->db;
		$q = $pdo->prepare($sql);
		$q->execute($valores);
